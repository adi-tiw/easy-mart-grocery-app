...
import { ItemData , products } from '@/data/products';
import { RetrieveItemData } from '@/data/apiProducts';
...
const Item = ({item}: ItemProps) => (
    ...
    <Pressable onPress={() => router.push({ pathname:'/pages/[id]' , params: item})}>
      ...
      <Text numberOfLines={1} style = {{fontFamily: 'poppins' , color:'#61AC4D', fontWeight: 'bold', fontSize: 13, margin: 2}}>{item.name}</Text>
      <Text style = {{fontFamily: 'poppins' , color:'grey', fontWeight: 'normal', fontSize: 10, margin: 2 , height: 27}}>{item.description}</Text>
